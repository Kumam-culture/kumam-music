...
// GET /api/playlists/user/mine — MUST be before /:id
...
router.get('/:id/songs', optionalAuth, async (req, res) => {
  try {
    const [playlists] = await pool.query('SELECT * FROM playlists WHERE uuid = ?', [req.params.id]);
    ...
    if (userId !== null) {
      const [rows] = await pool.query(`
        SELECT s.*, u.name AS artist_name, ap.stage_name, g.name AS genre_name,
          ...
          (SELECT COUNT(*) FROM likes WHERE user_id = ? AND song_id = s.id) AS liked
      ...
      const [rows] = await pool.query(`
        SELECT s.*, u.name AS artist_name, ap.stage_name, g.name AS genre_name,
          al.title AS album_title, al.artwork AS album_artwork, ps.position, 0 AS liked
        FROM playlist_songs ps
        JOIN songs s ON ps.song_id = s.id
        JOIN users u ON s.artist_id = u.id
        LEFT JOIN artist_profiles ap ON u.id = ap.user_id
...
router.post('/:id/songs', authenticate, async (req, res) => {
  try {
    ...
    const [songs] = await pool.query('SELECT id FROM songs WHERE uuid = ?', [song_uuid]);
    if (!songs.length) return res.status(404).json({ error: 'Song not found' });

    const [[max]] = await pool.query(
      'SELECT MAX(position) AS pos FROM playlist_songs WHERE playlist_id = ?',
      [playlists[0].id]
    );
    const position = (max.pos || 0) + 1;
    ...
    await pool.query(
      'UPDATE playlists SET total_songs = total_songs + 1 WHERE id = ?',
      [playlists[0].id]
    );
...
router.delete('/:id/songs/:songId', authenticate, async (req, res) => {
  try {
    const [playlists] = await pool.query(
      'SELECT * FROM playlists WHERE uuid = ? AND creator_id = ?',
      [req.params.id, req.user.id]
    );
    if (!playlists.length) return res.status(404).json({ error: 'Playlist not found or not authorized' });

    const [songs] = await pool.query('SELECT id FROM songs WHERE uuid = ?', [req.params.songId]);
    if (!songs.length) return res.status(404).json({ error: 'Song not found' });

    await pool.query(
      'DELETE FROM playlist_songs WHERE playlist_id = ? AND song_id = ?',
      [playlists[0].id, songs[0].id]
    );
    await pool.query(
      'UPDATE playlists SET total_songs = GREATEST(total_songs - 1, 0) WHERE id = ?',
      [playlists[0].id]
    );
