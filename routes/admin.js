...
router.get('/genres', ...adminOnly, async (req, res) => {
  try {
    ...
    const [genres] = await pool.query('SELECT * FROM genres ORDER BY group_id, sort_order');
...
// GET /api/genres (public — for dropdowns and browse)
router.get('/genre-groups', async (req, res) => {
  try {
    const [groups] = await pool.query('SELECT * FROM genre_groups ORDER BY sort_order');
    const [genres] = await pool.query('SELECT g.*, gg.name AS group_name FROM genres g LEFT JOIN genre_groups gg ON g.group_id = gg.id ORDER BY g.group_id, g.sort_order');
    res.json({ groups, genres });
  } catch (err) {
    res.status(500).json({ error: 'Server error' });
  }
