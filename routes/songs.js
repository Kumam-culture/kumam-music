...
router.post('/upload', authenticate, requireRole('artist'), (req, res, next) => {
  ...
  try {
    ...
    // ── Auto-inherit region & tribe from artist's profile ─────────
    // The artist already declared their region/tribe on registration.
    // Every song they upload is automatically grouped under those, regardless of song language.
    const [apRows] = await pool.query(
      'SELECT region_id, tribe_id FROM artist_profiles WHERE user_id = ?',
      [req.user.id]
    );
    const artistRegionId = apRows[0]?.region_id || null;
    const artistTribeId  = apRows[0]?.tribe_id  || null;
    ...
    const [result] = await pool.query(
      ...
      [uuid, req.user.id, albumIdVal, title, filePath, artworkPath,
       genreIdVal, artistRegionId, artistTribeId, lyrics || null, downloadable, premium, trackNum]
    );
...
router.get('/:id/download', authenticate, async (req, res) => {
  try {
    ...
    const filename   = `Etokwa_Music - ${artistName} - ${songTitle}.mp3`;
