...
router.get('/artist/my-songs', authenticate, requireRole('artist'), async (req, res) => {
  try {
    ...
    console.error('my-songs error:', err.sqlMessage || err.message);
...
router.get('/', optionalAuth, async (req, res) => {
  try {
    ...
    // Build SELECT liked expression (no ? in subquery when not logged in)
    ...
    // No ? params at all in dynamic part — only in LIMIT/OFFSET
    const sql = `
    ...
    const [songs] = await pool.query(sql, [limit, offset]);
    res.json({ songs });
  } catch (err) {
    console.error('GET /songs error:', err.sqlMessage || err.message);
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.post('/upload', authenticate, requireRole('artist'),
...
router.get('/:id/download', authenticate, async (req, res) => {
  try {
    const [songs] = await pool.query(
      'SELECT * FROM songs WHERE uuid = ? AND is_published = TRUE', [req.params.id]
    );
    if (!songs.length) return res.status(404).json({ error: 'Song not found' });
    const song = songs[0];
    ...
    // Sanitise artist name for filename
